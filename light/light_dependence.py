...
class LightDependent:
    ...
    def connect_to_ldd_for_ng(self, light, ldd, ng):
        ldd._light_info.setdefault(ng.name, []).append({
            "name" : light.name,
            "wavelength" : light.wavelength,
            "epsilon" : ldd.epsilon(light.wavelength),
            "area0" : light.light_model.area0,
        })

    def connected_light_info(self):
        return {k: list(v) for k, v in self._light_info.items()}
